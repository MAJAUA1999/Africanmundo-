...
function obterTexto(n){
  return n?.texto||n?.conteudo||n?.content||n?.descricao||"";
}

function obterImagem(n){
  return n?.imagem||n?.imagem_url||n?.image||n?.image_url||n?.url_imagem||"";
}
...
function abrirNoticia(n){
  if(!n?.id)return;
  location.href="noticia.html?id="+encodeURIComponent(n.id);
}

function abrirNoticiaPorId(id){
  if(!id)return;
  location.href="noticia.html?id="+encodeURIComponent(id);
}
...
function criarCard(n){
  ...
  card.innerHTML=`
    ${img?`
      <img src="${esc(img)}" alt="${esc(tit)}"
        loading="lazy"
        onerror="this.style.display='none'">
    `:`<div class="compact-img">🌍</div>`}
    <div class="compact-body">
      <div class="compact-cat">${esc(cat)}</div>
      <div class="compact-title">${esc(tit)}</div>
    </div>
  `;
...
function renderizarLista(id,lista){
  ...
  if(!Array.isArray(lista)||!lista.length){
    area.innerHTML=`<div class="loading">Ainda não existem notícias nesta categoria.</div>`;
    return;
  }

  lista.slice(0,4).forEach(n=>area.appendChild(criarCard(n)));
}
...
function renderizarDestaque(n){
  ...
  if(!n){
    area.innerHTML=`<div class="loading">Nenhuma notícia disponível.</div>`;
    return;
  }
    ...
    <article class="featured"
      ...
      ${img?`
        <img class="featured-image"
          src="${esc(img)}" alt="${esc(titulo)}"
          loading="lazy">
      `:`
        <div class="featured-image news-no-image">🌍</div>
      `}
      <div class="featured-content">
        <div class="featured-category">${esc(n.categoria||"Notícias")}</div>
        <h1 class="featured-title">${esc(titulo)}</h1>
        <p class="featured-text">
          ${esc(texto.slice(0,180))}${texto.length>180?"...":""}
        </p>
        <div class="featured-read">LER NOTÍCIA →</div>
      </div>
    </article>
  `;
}
...
async function carregarNoticias(){
  ...
  try{
    const resultado=await db.from("noticias")
      .select("*")
      .order("id",{ascending:false});

    if(resultado.error)throw resultado.error;

    window.__noticias=Array.isArray(resultado.data)?resultado.data:[];

    renderizarPagina();
    atualizarNotificacoes();

    console.log("✅ Notícias carregadas:",window.__noticias.length);

  }catch(e){
    console.error("❌ Erro ao carregar notícias:",e);
    mostrarErro(e.message||"Não foi possível carregar as notícias.");
  }
}

function renderizarPagina(){
  const noticias=Array.isArray(window.__noticias)?[...window.__noticias]:[];
  if(!noticias.length)return;

  const embaralhar=lista=>lista
    .map(n=>({n,ordem:Math.random()}))
    .sort((a,b)=>a.ordem-b.ordem)
    .map(x=>x.n);

  const destaque=noticias[Math.floor(Math.random()*noticias.length)];
  renderizarDestaque(destaque);
  renderizarLista("ultimas",noticias.slice(0,4));

  [
    "futebol","mocambique","africa",
    "negocios","entretenimento","desporto"
  ].forEach(cat=>{
    const lista=noticias.filter(n=>
      normalizarCategoria(n.categoria)===cat
    );
    renderizarLista(cat,embaralhar(lista));
  });
}
...
function pesquisar(e){
  ...
  if(!resultados.length){
    area.innerHTML=`<div class="loading">🔎 Nenhuma notícia encontrada.</div>`;
    return;
  }

  const titulo=document.createElement("h3");
  titulo.textContent="🔎 Resultados da pesquisa";
  area.appendChild(titulo);

  const grid=document.createElement("div");
  grid.className="news-grid";

  resultados.slice(0,8).forEach(n=>grid.appendChild(criarCard(n)));
...
function guardarFavorito(n){
  ...
  if(favoritos.some(x=>String(x.id)===String(n.id))){
    abrirModal("❤️ Favoritos","<p>Esta notícia já está nos favoritos.</p>");
    return;
  }
  ...
  abrirModal("❤️ Favoritos","<p>Notícia guardada com sucesso!</p>");
}
...
function abrirFavoritos(){
  ...
  const html=favoritos.map((n,i)=>`
    ...
    ">
      <strong>${esc(obterTitulo(n))}</strong><br>
      <button onclick="abrirNoticiaPorId('${esc(n.id)}')" style="margin-top:8px">
        LER →
      </button>
      <button onclick="removerFavorito(${i})" style="margin-top:8px">
        🗑️
      </button>
    </div>
  `).join("");
...
function atualizarNotificacoes(){
  ...
  if(!btn)return;
  btn.querySelector(".notification-count")?.remove();
}
...
function abrirNotificacoes(){
  ...
  if(
    ...
    botaoPush=Notification.permission==="granted"
      ?`<div style="
  ...
  const html=noticias.map(n=>`
    <button type="button"
      ...
      <strong>${esc(obterTitulo(n))}</strong>
    </button>
  `).join("");

  abrirModal(
    "🔔 Últimas novidades",
    botaoPush+(html||"<p>Nenhuma novidade.</p>")
  );
}
...
async function ativarNotificacoesPush(){
  ...
  try{
    ...
    const resposta=await fetch(
      SUPABASE_URL+"/functions/v1/salvar-push",
      {
        method:"POST",
        headers:{
    ...
    if(!resposta.ok||!resultado.sucesso){
      throw new Error(
        resultado.erro||"Erro ao guardar inscrição."
      );
    }
...
function abrirUsuario(){
  abrirModal("👤 Minha área",`
    <div style="text-align:center">
      ...
      <p style="color:var(--muted)">
        Guarda notícias favoritas e personaliza a tua experiência.
      </p>
      <button onclick="abrirFavoritos()">❤️ Meus favoritos</button>
    </div>
  `);
...
function salvarSite(){
  abrirModal("⭐ Guardar AfricanMundo",`
    <div style="text-align:center">
      <div style="font-size:45px">⭐</div>
      <h3>Guardar AfricanMundo</h3>
      <p style="color:var(--muted)">
...
async function carregarAnunciosAtivos(){
  ...
  try{
    ...
    const anunciosValidos=(data||[]).filter(a=>{
      const inicio=a.data_inicio
        ?new Date(a.data_inicio+"T00:00:00"):null;

      const fim=a.data_fim
        ?new Date(a.data_fim+"T23:59:59"):null;
    ...
    area.innerHTML=`
      <div style="
        ...
        box-sizing:border-box;
      ">
        ${anunciosValidos.map(a=>{

          const empresa=esc(a.empresa||"Publicidade");
          const mensagem=esc(a.mensagem||"");
          const imagem=a.imagem?esc(a.imagem):"";
          const video=esc(a.video||a["vídeo"]||"");
          const link=a.link?esc(a.link):"";
          ...
          `:imagem?`
            <img src="${imagem}" alt="${empresa}" loading="lazy"
              style="
                display:block;width:100%;
                max-width:100%;height:60px;
                object-fit:cover;
